...
export default Vue.extend<ModalRootData, any, any, any>({
  ...
  methods: {
    ...
    switchPrevNext() {
      ...
      if (prevModalState && nextIsPage) {
        ...
        if (prevModalState.translateY && nextModalState.translateYFrom)
          if (prevIsPage && prevModalState.translateY <= nextModalState.translateYFrom && !this.isBack) {
    ...
    onMaskClick() {
      ...
      if (!this.switching && !this.checkBooleanAttrTrue(this.modalsState[activeModalName].persistent)) {
    ...
    getModalPropsByNode(node: VNode): any {
      if (node.componentOptions !== undefined && node.componentOptions.propsData !== undefined) {
    ...
    getModalOnCloseByNode(node: VNode): () => any {
      if (node.componentOptions !== undefined && node.componentOptions.listeners !== undefined) {
      ...
      this.maskAnimationFrame = requestAnimationFrame(() => {
        if (this.$refs.maskElementRef) {
          ...
          if ((translateY || translateY === 0) && translateYCurrent) {
            const opacity =
              forceOpacity === undefined ? 1 - (translateYCurrent - translateY) / (100 - translateY) || 0 : forceOpacity
            this.$refs.maskElementRef.style.opacity = Math.max(0, Math.min(100, opacity)).toString()
          }
        }
      })
    },
    ...
    animateTranslate(modalState: ModalsStateEntry, currentPercent: number | null = null) {
      ...
      this.frameIds[frameId] = requestAnimationFrame(() => {
        if (modalState.innerElement) {
          setTransformStyle(modalState.innerElement, `translateY(${proxyCurrentPercent}%)`)
        }
        ...
        if (
          ...
          const factor = modalState.innerElement.offsetHeight * (proxyCurrentPercent / 100)
    ...
    animatePageHeader(modalState: ModalsStateEntry, currentPercent: number | null = null) {
      ...
      if (proxyCurrentPercent !== null) {
        const headerOpenPercent = proxyCurrentPercent < 15 ? Math.max(0, 15 - proxyCurrentPercent) / 15 : 0
    ...
    onCardTouchEnd(e: any, modalState: ModalsStateEntry) {
      ...
      if (this.dragging) {
        let translateY = proxyModalState.translateYCurrent ? proxyModalState.translateYCurrent : 0
        const translateYPercent = proxyModalState.touchShiftYPercent ? proxyModalState.touchShiftYPercent : 0
        const touchStartTime: any = proxyModalState.touchStartTime ? proxyModalState.touchStartTime : new Date()

        const expectTranslateY =
          translateY - (Date.now() - touchStartTime.getTime()) * 240 * 0.6 * (translateYPercent < 0 ? -1 : 1)
    ...
    initPageModal(modalState: ModalsStateEntry, modalElement: HTMLElement) {
      ...
      const contentElement: HTMLElement | null = modalElement.querySelector('.vc-ModalPage__content')
      const contentHeight = contentElement ? (contentElement.firstElementChild as HTMLElement).offsetHeight : 0

      const prevTranslateY = modalState.translateY

      const innerElement: HTMLElement | null = modalElement.querySelector('.vc-ModalPage__in-wrap')
      const headerElement: HTMLElement | null = modalElement.querySelector('.vc-ModalPage__header')
      const contentQueryElement: HTMLElement | null = modalElement.querySelector('.vc-ModalPage__content')
      const footerElement: HTMLElement | null = modalElement.querySelector('.vc-ModalPage__footer')

      newModalState.expandable = contentHeight > (contentElement ? contentElement.clientHeight : 0)
      ...
      newModalState.contentElement = contentQueryElement !== null ? contentQueryElement : undefined
      ...
      if (newModalState.expandable) {
        translateYFrom = 100 - (newModalState.settlingHeight ? newModalState.settlingHeight : 75)
        ...
        const headerHeight = newModalState.headerElement ? newModalState.headerElement.offsetHeight : 0
    ...
    onPageTouchMove(e: any, modalState: ModalsStateEntry) {
      ...
      if (
        ...
        collapsed ||
        (expanded && proxyModalState.touchMovePositive && proxyModalState.touchStartContentScrollTop === 0) ||
    ...
    onPageTouchEnd(e: any, modalState: ModalsStateEntry) {
      ...
      if (this.dragging) {
        ...
        const touchStartTime = modalState.touchStartTime ? modalState.touchStartTime : new Date()
        const touchShiftYPercent = modalState.touchShiftYPercent ? modalState.touchShiftYPercent : new Date()

        let translateY = proxyModalState.translateYCurrent ? proxyModalState.translateYCurrent : 0
        const expectTranslateY =
          (translateY / (Date.now() - touchStartTime.getTime())) * 240 * 0.6 * (touchShiftYPercent < 0 ? -1 : 1)
        translateY = rangeTranslate(translateY + expectTranslateY)

        const expandedRange = proxyModalState.expandedRange ? proxyModalState.expandedRange : [0, 0]
        const collapsedRange = proxyModalState.collapsedRange ? proxyModalState.collapsedRange : [0, 0]
        const hiddenRange = proxyModalState.hiddenRange ? proxyModalState.hiddenRange : [0, 0]
        ...
        } else if (numberInRange(translateY, hiddenRange) && !this.checkBooleanAttrTrue(proxyModalState.persistent)) {
          translateY = 100
        } else {
          translateY = proxyModalState.translateYFrom ? proxyModalState.translateYFrom : 0
        }

        if (translateY !== 100 && shiftYEndPercent >= 75 && !this.checkBooleanAttrTrue(proxyModalState.persistent)) {
          translateY = 100
        }
      ...
      <touch class={this.classNames} onMoveY={this.onTouchMove} onEnd={this.onTouchEnd}>
        ...
        <div class='vc-ModalRoot__viewport'>
          {this.modals.map((modal: VNode) => {
            ...
            return (
              <div class={this.calculateModalClass(modalId, modalState, isPage)} id={key} key={key}>
                {modal}
              </div>
            )
          })}
