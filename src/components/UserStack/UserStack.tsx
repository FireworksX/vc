      ...
      <div class={this.classNames}>
        <div class='vc-UsersStack__photos'>
          {photosShown.map((photo: string, i: number) => {
            return <div key={i} class='vc-UsersStack__photo' style={{ backgroundImage: `url(${photo})` }} />
          })}

          {canShowOthers && <div class='vc-UsersStack__photo vc-UsersStack__photo--others'>{`+${othersCount}`}</div>}
        </div>
        {this.$slots.default && <div class='vc-UsersStack__text'>{this.$slots.default}</div>}
      </div>
    )
  }
})
